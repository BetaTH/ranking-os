...
export function Dashboard (){
    ...
    interface transform2Rank {
        [key : string] : {equipe: string, pontos: number, classTransporte: string,fotoLink: string,}
    }
    
    useEffect(() => {
        let queryParams = { dateMin : "07/01/2022 00:00", dateMax : "07/13/2022 23:59"};
        let url = new URL("http://localhost:5000/teste");
        let k: keyof typeof queryParams
        for (k in queryParams) { url.searchParams.append(k, queryParams[k])}
        fetch(url)
